...
def calculate_split_info(attribute_indecies, row_numbers):
    ''' 
        Calculate the split information of all attributes in complete dataset
        This function is called only once in the beginning
    '''
    # Some inits
    __m.split_infos = [None for i in range(len(attribute_indecies))]
    ...
    for attr in attribute_indecies:        
        ...
        for val in counter:
            ...
            split_information += (-val_prob) * math.log2(val_prob)

        # add split information
        __m.split_infos[attr] = split_information
...
def create_dtree(data, attr_details, attribute_indecies, target_attr, confidence):
    ...
    calculate_split_info(attribute_indecies, __m.row_numbers)

    # Init mt pools
    dtree_mt_init()

    cleaned_attributes = [ attr for attr in attribute_indecies if __m.split_infos[attr] > 0.0 ]

    # create the tree
    dtree = grow_dtree(cleaned_attributes, __m.row_numbers)
...
def process_data(row_numbers, attribute_indecies):
    ...
    for attr in attribute_indecies:        
        ...
        gain_ratio = node_info_gain / __m.split_infos[attr]
